...
- **后端**：FastAPI + LangGraph 的聊天 / 会话 / 事件推送服务
- **前端**：React + Vite 的聊天界面
- **检索能力**：保留 Markdown 分块、向量检索、Rerank、检索工具封装和数据导入脚本

当前主功能以聊天、配方生成/优化与检索能力为主，不再暴露旧的知识库管理与上传接口。
...
- 当前前端聚焦聊天、会话和右侧辅助面板
- 旧的知识库管理 / 配方知识库管理 / 上传页面已删除
- `useKnowledgeBase` 现在只保留本地状态壳层，不再向后端请求知识库管理接口

因此，凡是涉及前端功能说明，都不要再写“可在 UI 中管理知识库/上传文件”，除非相关代码和路由已恢复
